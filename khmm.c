...
FLOAT hmm_Viterbi(const hmm_par_t *hp, hmm_data_t *hd)
{
	...
	for (u = 2; u <= hd->L; ++u) {
		...
		for (k = 0; k != hp->n; ++k) {
			FLOAT *laa = la[k];
			for (l = 0, max = -HMM_INF, max_l = -1; l != hp->n;
			     ++l) {
				if (max < preV[l] + laa[l]) {
					max = preV[l] + laa[l];
					max_l = l;
...
void hmm_forward(const hmm_par_t *hp, hmm_data_t *hd)
{
	...
	for (u = 2; u <= L; ++u) {
		FLOAT *fu = hd->f[u], *fu1 = hd->f[u - 1],
		      *eu = hp->e[(int)hd->seq[u]];
		for (k = 0, sum = 0.0; k != n; ++k) {
			FLOAT *aa = at[k];
...
void hmm_backward(const hmm_par_t *hp, hmm_data_t *hd)
{
	...
	if (tmp > 1.0 + 1e-6 ||
	    tmp < 1.0 - 1e-6) // in theory, tmp should always equal to 1
...
FLOAT hmm_post_state(const hmm_par_t *hp, const hmm_data_t *hd, int u,
		     FLOAT *prob)
{
...
hmm_exp_t *hmm_expect(const hmm_par_t *hp, const hmm_data_t *hd)
{
	...
	for (u = 1; u < hd->L; ++u) {
		FLOAT *fu = hd->f[u], *bu = hd->b[u], *bu1 = hd->b[u + 1],
		      ss = hd->s[u];
		FLOAT *Ec = he->E[(int)hd->seq[u]],
		      **p = hp->ae + (int)hd->seq[u + 1] * n;
		for (k = 0; k != n; ++k) {
	...
	for (l = 0; l != n; ++l)
		he->A0[l] +=
			hp->a0[l] * hp->e[(int)hd->seq[1]][l] * hd->b[1][l];
	return he;
}
