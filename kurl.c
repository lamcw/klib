...
#define KU_MAX_SKIP                                                            \
	(KU_DEF_BUFLEN << 1) // if seek step is smaller than this, skip
...
static size_t write_cb(char *ptr, size_t size, size_t nmemb,
		       void *data) // callback required by cURL
{
...
{
	assert(ku->p_buf ==
	       ku->l_buf); // buffer is always used up when fill_buffer() is called; otherwise a bug
	ku->off0 += ku->l_buf;
	ku->p_buf = ku->l_buf = 0;
	if (ku->done_reading)
		return 0;
	if (kurl_isfile(ku)) {
		...
		while (ku->l_buf < ku->m_buf) {
			int l;
			l = read(ku->fd, ku->buf + ku->l_buf,
				 ku->m_buf - ku->l_buf);
			if (l == 0)
				break;
			ku->l_buf += l;
		}
		if (ku->l_buf < ku->m_buf)
			ku->done_reading = 1;
	} else {
		int n_running, rc;
		fd_set fdr, fdw, fde;
		do {
			...
			FD_ZERO(&fdw);
			FD_ZERO(&fde);
			curl_multi_fdset(ku->multi, &fdr, &fdw, &fde,
					 &maxfd); // FIXME: check return code
			if (maxfd >= 0 &&
			...
			if (maxfd <
				...
				req.tv_sec = 0;
				req.tv_nsec = 100000000; // this is 100ms
				nanosleep(&req, &rem);
			}
			curl_easy_pause(ku->curl, CURLPAUSE_CONT);
			rc = curl_multi_perform(
				ku->multi,
				&n_running); // FIXME: check return code
		} while (n_running &&
			 ku->l_buf < ku->m_buf - CURL_MAX_WRITE_SIZE);
		if (ku->l_buf < ku->m_buf - CURL_MAX_WRITE_SIZE)
			ku->done_reading = 1;
	}
	return ku->l_buf;
}
...
{
	extern s3aux_t s3_parse(const char *url, const char *_id,
				const char *_secret, const char *fn);
	const char *p, *q;
	kurl_t *ku;
	...
	if (is_file && (fd = open(url, O_RDONLY)) < 0)
		return 0;
	...
	if (!kurl_isfile(ku)) {
		...
		if (strstr(url, "s3://") == url) {
			s3aux_t a;
			a = s3_parse(url, (opt ? opt->s3keyid : 0),
				     (opt ? opt->s3secretkey : 0),
				     (opt ? opt->s3key_fn : 0));
	...
	if (kurl_isfile(ku))
		failed = (fill_buffer(ku) <= 0);
	else
		failed = (prepare(ku, 0) < 0 || fill_buffer(ku) <= 0);
	if (failed) {
		kurl_close(ku);
		return 0;
	}
...
{
	if (len <= 0 || len < ku->l_buf)
		return ku->m_buf;
	if (!kurl_isfile(ku) && len < CURL_MAX_WRITE_SIZE * 2)
		return ku->m_buf;
	ku->m_buf = len;
	kroundup32(ku->m_buf);
...
ssize_t kurl_read(kurl_t *ku, void *buf, size_t nbytes)
{
	...
	while (rest) {
		if (ku->l_buf - ku->p_buf >= rest) {
			if (buf)
				memcpy((uint8_t *)buf + (nbytes - rest),
				       ku->buf + ku->p_buf, rest);
			ku->p_buf += rest;
			rest = 0;
		} else {
			int ret;
...
off_t kurl_seek(
	kurl_t *ku, off_t offset,
	int whence) // FIXME: sometimes when seek() fails, read() will fail as well.
{
	...
	if (whence == SEEK_SET)
		...
		new_off = lseek(ku->fd, offset, SEEK_END), seek_end = 1;
	...
	if (!seek_end && new_off >= cur_off &&
	    new_off - cur_off + ku->p_buf < ku->l_buf) {
		ku->p_buf += new_off - cur_off;
		return ku->off0 + ku->p_buf;
	}
	if (seek_end || new_off < cur_off ||
	    new_off - cur_off >
		    KU_MAX_SKIP) { // if jump is large, do actual seek
		ku->off0 = new_off;
		ku->done_reading = 0;
		if (prepare(ku, 1) < 0 || fill_buffer(ku) <= 0)
			failed = 1;
	...
	if (failed)
		ku->err = KURL_SEEK_OUT, ku->l_buf = ku->p_buf = 0,
		new_off = -1;
	return new_off;
}
...
{
	const uint8_t table[] = { 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd,
...
{
	uint32_t i, t, a = s->state.w[0], b = s->state.w[1], c = s->state.w[2],
		       d = s->state.w[3], e = s->state.w[4];
	for (i = 0; i < 80; i++) {
		if (i >= 16) {
			t = s->buf.w[(i + 13) & 15] ^ s->buf.w[(i + 8) & 15] ^
			    s->buf.w[(i + 2) & 15] ^ s->buf.w[i & 15];
			s->buf.w[i & 15] = rol32(t, 1);
		}
		if (i < 20)
			t = 0x5a827999 + (d ^ (b & (c ^ d)));
		else if (i < 40)
			t = 0x6ed9eba1 + (b ^ c ^ d);
		else if (i < 60)
			t = 0x8f1bbcdc + ((b & c) | (d & (b | c)));
		else
			t = 0xca62c1d6 + (b ^ c ^ d);
...
const uint8_t *sha1_final(sha1nfo *s)
{
	...
	for (i = 0; i < 5; ++i) {
		...
		s->state.w[i] = a << 24 | (a << 8 & 0x00ff0000) |
				(a >> 8 & 0x0000ff00) | a >> 24;
	}
	return s->state.b;
}
...
const uint8_t *sha1_final_hmac(sha1nfo *s)
{
	...
	sha1_init(s);
	for (i = 0; i < BLOCK_LENGTH; ++i)
		sha1_write1(s, s->keyBuffer[i] ^ HMAC_OPAD);
	for (i = 0; i < HASH_LENGTH; ++i)
		sha1_write1(s, s->innerHash[i]);
	return sha1_final(s);
}
...
static void s3_sign(const char *key, const char *data, char out[29])
{
	...
	for (j = i = 0, rest = 8; i < 20; ++j) { // base64 encoding
		if (rest <= 6) {
			...
			out[j] = b64tab[(int)(digest[i] << (6 - rest) & 0x3f) |
					next >> (rest + 2)],
			++i, rest += 2;
		} else
			out[j] = b64tab[(int)digest[i] >> (rest - 6) & 0x3f],
			rest -= 6;
	}
	out[j++] = '=';
	out[j] =
...
s3aux_t s3_parse(const char *url, const char *_id, const char *_secret,
		 const char *fn_secret)
{
	...
	if (_id == 0 || _secret == 0) {
		...
		if (id_secret == 0)
			return a; // fail to read the AWS credential
		id = id_secret;
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "c:l:a:")) >= 0) {
		if (c == 'c')
			start = strtol(optarg, &p, 0);
		else if (c == 'l')
			rest = strtol(optarg, &p, 0);
		else if (c == 'a')
			opt.s3key_fn = optarg;
	}
	if (optind == argc) {
