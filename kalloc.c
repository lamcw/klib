...
static header_t *morecore(kmem_t *km, size_t nu)
{
	...
	*p = nu -
	...
	kfree(km,
	      p + 1); /* initialize the new "core"; NB: the core header is not looped. */
	return km->loop_head;
}
	   ...
	   void *ap) /* kfree() also adds a new core to the circular list */
{
	...
	if (q + q->size ==
		...
		km->loop_head = p,
		q->ptr =
			p; /* in two cores, cannot be merged; create a new block in the list */
}
...
void *kmalloc(void *_km, size_t n_bytes)
{
	...
	for (p = q->ptr;; q = p, p = p->ptr) { /* search for a suitable block */
		if (p->size >=
			...
			if (p->size == n_units)
				q->ptr =
					p->ptr; /* no need to split the block */
			else { /* split the block. NB: memory is allocated at the end of the block! */
				p->size -=
					n_units; /* reduce the size of the free block */
