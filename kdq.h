...
#define __KDQ_IMPL(type, SCOPE)                                                \
	...
	{                                                                      \
		size_t new_size = 1ULL << new_bits,                            \
		       old_size = 1ULL << q->bits;                             \
		...
		if (new_bits > q->bits)                                        \
			q->a = (type *)realloc(q->a, (1ULL << new_bits) *      \
							     sizeof(type));    \
		if (q->front + q->count <= old_size) { /* unwrapped */         \
			if (q->front + q->count >                              \
			    new_size) /* only happens for shrinking */         \
	...
	{                                                                      \
		return q->count ? &q->a[((--q->count) + q->front) & q->mask] : \
				  0;                                           \
	}                                                                      \
