...
static int http_open(const char *fn)
{
	...
	if (proxy == 0) {
		host = strdup(
			http_host); // when there is no proxy, server name is identical to http_host name.
		port = strdup(*q ? q : "80");
		path = strdup(*p ? p : "/");
	} else {
...
static int kftp_get_response(ftpaux_t *aux)
{
	...
	while (read(aux->ctrl_fd, &c,
		...
		if (n >= aux->max_response) {
			aux->max_response = aux->max_response ?
						    aux->max_response << 1 :
						    256;
			aux->response =
				realloc(aux->response, aux->max_response);
		}
		aux->response[n++] = c;
		if (c == '\n') {
