...
kson_node_t *kson_parse_core(const char *json, long *_n, int *error,
			     long *parsed_len)
{
...
#define __new_node(z)                                                          \
	do {                                                                   \
		if (n_a == m_a) {                                              \
			long old_m = m_a;                                      \
			m_a = m_a ? m_a << 1 : 4;                              \
			a = (kson_node_t *)realloc(a,                          \
						   sizeof(kson_node_t) * m_a); \
			memset(a + old_m, 0,                                   \
			       sizeof(kson_node_t) * (m_a - old_m));           \
		}                                                              \
		*(z) = &a[n_a++];                                              \
	} while (0)
	...
	*error = KSON_OK;
	for (p = json; *p; ++p) {
		...
		if (*p == ',') { // comma is somewhat redundant
			...
			int t = *p == '[' ? -1 : -2;
			if (top < 2 ||
			    stack[top - 1] != -3) { // unnamed internal node
				__push_back(n_a);
				__new_node(&u);
				__push_back(t);
			} else
				stack[top - 1] = t; // named internal node
		} else if (*p == ']' || *p == '}') {
			...
			u->key = u->v.str;
			u->n = top - 1 - start;
			u->v.child = (kson_node_t **)malloc(
				u->n * sizeof(kson_node_t *));
			tmp = (size_t *)u->v.child;
			for (i = start + 1; i < top; ++i)
				tmp[i - start - 1] = stack[i];
			u->type =
				*p == ']' ? KSON_TYPE_BRACKET : KSON_TYPE_BRACE;
			if ((top = start) == 1)
				break; // completed one object; remaining characters discarded
		} else if (*p == ':') {
			...
			if (top >= 2 &&
			    stack[top - 1] ==
				    -3) { // we have a key:value pair here
				--top;
				u = &a[stack[top - 1]];
			...
			u->v.str = (char *)malloc(q - p + 1);
			strncpy(u->v.str, p, q - p);
			u->v.str[q - p] =
				0; // equivalent to u->v.str=strndup(p, q-p)
			u->type = c == '\'' ? KSON_TYPE_SGL_QUOTE :
					      c == '"' ? KSON_TYPE_DBL_QUOTE :
							 KSON_TYPE_NO_QUOTE;
			p = c == '\'' || c == '"' ? q : q - 1;
		}
	}
	...
	if (parsed_len)
		*parsed_len = p - json;
	if (top != 1)
		*error = KSON_ERR_EXTRA_LEFT;

	for (i = 0; i < n_a; ++i)
		for (j = 0, u = &a[i], tmp = (size_t *)u->v.child;
		     j < (long)u->n; ++j)
			u->v.child[j] = &a[tmp[j]];
...
void kson_destroy(kson_t *kson)
{
	...
	for (i = 0; i < kson->n_nodes; ++i) {
		free(kson->root[i].key);
		free(kson->root[i].v.str);
	}
	free(kson->root);
	free(kson);
}
				...
				if (argc > 2) {
					...
					if (p) {
						if (kson_is_internal(p))
							printf("Reached an internal node\n");
						else
							printf("Value: %s\n",
							       p->v.str);
					} else
						printf("Failed to find the slot\n");
				}
			} else
