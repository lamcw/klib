...
// from bowtie-0.9.8.1
inline static int bt1_pop64(
	uint64_t x) // the kbi_popcount64() equivalence; similar to popcount_2() in wiki
{
...
inline static int bt1_countInU64(uint64_t dw,
				 int c) // the kbi_DNAcount64() equivalence
{
...
int main(void)
{
	...
	fprintf(stderr, "%20s\t%20ld\t%10.6f\n", "__builtin_popcountl",
		(long)cnt, (double)(clock() - t) / CLOCKS_PER_SEC);
