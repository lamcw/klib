...
namespace klib
{
template <class T, typename Less = std::less<T> > class Avl {
	...
	inline Node *rotate1(Node *p, int dir)
	...
	inline Node *rotate2(Node *p, int dir)
	{
		...
		b1 = dir == 0 ? +1 : -1;
		if (r->balance == b1)
			q->balance = 0, p->balance = -b1;
		else if (r->balance == 0)
			q->balance = p->balance = 0;
		else
			q->balance = b1, p->balance = 0;
		r->balance = 0;
		return r;
	};
	void destroy(Node *r)
	...
	T *insert(const T &data, bool *is_new = NULL, unsigned *cnt_ = NULL)
	{
		...
		if (cnt_)
			*cnt_ = cnt;
		x = new Node;
		x->data = data, x->balance = 0, x->size = 1,
		x->p[0] = x->p[1] = 0;
		if (is_new)
			*is_new = true;
		if (q == 0)
		...
		for (i = 0; i < path_len; ++i)
			++path[i]->size;
		for (p = bp, top = 0; p != x;
		     p = p->p[stack[top]], ++top) /* update balance factors */
			if (stack[top] == 0)
				--p->balance;
			else
				++p->balance;
		if (bp->balance > -2 && bp->balance < 2)
			return &x->data; /* no re-balance needed */
		// re-balance
		which = (bp->balance < 0);
		b1 = which == 0 ? +1 : -1;
		q = bp->p[1 - which];
