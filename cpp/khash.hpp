...
class KHash {
	...
	KHash()
		: n_buckets(0), count(0), n_occupied(0), upper_bound(0),
		  flags(NULL), keys(NULL){};
	~KHash()
	{
		std::free(flags);
		std::free(keys);
	};
	khint_t capacity(void) const
	{
		return n_buckets;
	};
	...
	khint_t begin(void) const
	{
		return 0;
	};
	khint_t end(void) const
	{
		return n_buckets;
	};

	void exist(khint_t x) const
	{
		...
		khint_t j = 1;
		{
			...
			if (count >=
				...
				if (n_buckets < new_n_buckets) { /* expand */
					T *new_keys = (T *)std::realloc(
						(void *)keys,
						new_n_buckets * sizeof(T));
					if (!new_keys) {
						std::free(new_flags);
						return -1;
					}
	...
	khint_t put(const T &key, int *ret)
	{
		...
		if (n_occupied >= upper_bound) { /* update the hash table */
			if (n_buckets > (count << 1)) {
				if (resize(n_buckets - 1) <
				    0) { /* clear "deleted" elements */
					*ret = -1;
					return n_buckets;
				}
			} else if (resize(n_buckets + 1) <
		...
		{
			khint_t k, i, site, last, mask = n_buckets - 1,
						  step = 0;
			x = site = n_buckets;
			k = Hash()(key);
			i = k & mask;
